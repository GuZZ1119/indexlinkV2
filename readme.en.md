...
| Restricted DSL, deterministic runtime, Studio, and admission | Represents only allow-listed indicators, bounded expressions, and opportunity actions. Saving rebuilds domain invariants; activation compares a fixed-fixture backtest and checks budget/core-bucket safety. Close, SMA, EMA, RSI, drawdown, and VIX now use the same causal Decimal builder for live simulation and offline fixture tests. Admission still covers RSI(14)/VIX only: other indicators remain ineligible until their complete historical evidence is admitted, rather than fabricating a backtest. |
