...
| 策略中心 / Strategy Center | 以 Fixed DCA 为基准，按方法家族理解 100 个不可变 Formula 参数预设，搜索、筛选、分析并采用精确版本 | `/strategy-center` 读取 `GET /strategy-catalog`，按 `family` 聚合为 20 个入口、在家族内选择 `preset`；自适应 70/20/10 暂不进入普通入口，不把受限 DSL 暴露为用户概念 |
