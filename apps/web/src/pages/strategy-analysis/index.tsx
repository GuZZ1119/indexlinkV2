...
import { ApiRequestError, useStrategyBacktest } from '@/api/queries'
import type { DynamicBacktestSeries, StrategyBacktestRequest } from '@/api/types'
import { PageHeading } from '@/components/v2_1/page-heading'
import { StrategyCenterNav } from '@/components/v2_1/strategy-center-nav'
import { consumerStrategies, findConsumerStrategy, strategyAnalysisRanges, type StrategyId } from '@/features/v2_1/model'
import { openStrategyAnalysis, strategyAnalysisStore, submitStrategyAnalysis, toggleAnalysisStrategy } from '@/stores/ui'

const colors: Record<string, string> = {
  fixed_dca: '#50738a',
  dsl_ma200_trend_guard: '#2d6a57',
  dsl_growth_volatility_balance: '#ad7d35',
}

const strategyIdByPolicy: Record<string, StrategyId> = {
  fixed_dca: 'steady-dca',
  dsl_ma200_trend_guard: 'ma200-trend-guard',
  dsl_growth_volatility_balance: 'growth-volatility-balance',
}
...
export default function StrategyAnalysisPage() {
  ...
  useEffect(() => {
    ...
    if (requestedStrategy) openStrategyAnalysis(requestedStrategy)
  }, [requestedStrategy, requestedView])
  ...
  const backtest = useStrategyBacktest(request)
  ...
  return (
    <div className="mx-auto w-full max-w-7xl space-y-8 px-5 py-8 md:px-8 lg:px-10 lg:py-10">
      ...
      {backtest.isLoading ? <LoadingState /> : backtest.isError ? <ErrorState error={backtest.error} /> : result ? <>
        <ProvenanceStrip response={result} />
        {state.view === 'plain'
          ? <PlainView series={result.result.series} chartData={chartData} currency={result.data.currency} selected={state.strategyIds as readonly StrategyId[]} />
          : <ResearchView series={result.result.series} currency={result.data.currency} effectiveStart={result.result.effective_start} effectiveEnd={result.result.effective_end} contributionCount={result.result.contribution_count} />}
      </> : null}
    </div>
  )
}

function PlainView({ series, chartData, currency, selected }: { series: DynamicBacktestSeries[]; chartData: Array<Record<string, string | number>>; currency: string; selected: readonly StrategyId[] }) {
  return <section className="rounded-[1.45rem] border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
    <div className="grid gap-6 lg:grid-cols-[15rem_minmax(0,1fr)]">
      <aside><p className="text-sm font-semibold text-[#102028]">选择要对比的策略</p><p className="mt-1 text-xs leading-5 text-slate-500">改变选择后，点击上方“运行真实回测”。</p><div className="mt-4 space-y-2">{consumerStrategies.map((strategy) => { const active = selected.includes(strategy.id); return <button key={strategy.id} type="button" aria-pressed={active} onClick={() => toggleAnalysisStrategy(strategy.id)} className={`flex w-full items-center gap-3 rounded-xl border p-3 text-left transition ${active ? 'border-[#a9cbbb] bg-[#f1f7f4] shadow-[0_8px_24px_rgb(45_106_87_/_0.08)]' : 'border-slate-200 hover:border-slate-300'}`}><span className="grid size-5 shrink-0 place-items-center rounded-full border" style={{ borderColor: colors[policyId(strategy.id)], backgroundColor: active ? colors[policyId(strategy.id)] : 'transparent' }}>{active && <Check className="size-3.5 text-white" />}</span><span><span className="block text-sm font-medium text-[#102028]">{strategy.shortName}</span><span className="mt-0.5 block text-xs text-slate-500">{strategy.cadence}</span></span></button> })}</div></aside>
      <div className="min-w-0"><div className="flex flex-wrap items-end justify-between gap-3"><div><p className="inline-flex items-center gap-2 text-sm font-medium text-[#2d6a57]"><BarChart3 className="size-4" />真实归一化走势</p><h2 className="mt-2 text-2xl font-semibold tracking-[-0.035em] text-[#102028]">共同起点 = 100</h2></div><p className="max-w-md text-xs leading-5 text-slate-500">这里只比较策略造成的路径差异；金额、币种和投入次数不会让某条线凭空领先。</p></div><div className="mt-5 h-[23rem] rounded-[1.2rem] bg-[#f8faf9] p-3 sm:p-5"><ResponsiveContainer width="100%" height="100%" minWidth={0} minHeight={1}><LineChart data={chartData} margin={{ top: 8, right: 12, left: -12, bottom: 0 }}><CartesianGrid vertical={false} stroke="#dce7e1" strokeDasharray="3 3" /><XAxis dataKey="date" tickLine={false} axisLine={false} minTickGap={38} tick={{ fill: '#718096', fontSize: 12 }} /><YAxis domain={['auto', 'auto']} tickLine={false} axisLine={false} width={46} tickFormatter={(value) => Number(value).toFixed(0)} tick={{ fill: '#718096', fontSize: 12 }} /><Tooltip cursor={{ stroke: '#b8d5c6', strokeDasharray: '3 3' }} contentStyle={{ borderRadius: 14, borderColor: '#d8e5df', boxShadow: '0 10px 30px rgb(15 32 40 / 0.08)' }} formatter={(value, name) => [`${Number(value).toFixed(2)} 指数`, `${name}`]} />{series.map((item) => <Line key={item.strategy_id} type="monotone" dataKey={item.strategy_id} name={displayName(item.strategy_id)} stroke={colors[item.strategy_id] ?? '#64748b'} strokeWidth={3} dot={false} activeDot={{ r: 4 }} />)}</LineChart></ResponsiveContainer></div></div>
    </div>
    <div className="mt-6 grid gap-3 border-t border-slate-100 pt-6 sm:grid-cols-2 xl:grid-cols-3">{series.map((item) => <article key={item.strategy_id} className="rounded-xl border border-slate-200 bg-[#fbfcfb] p-4"><div className="flex items-center justify-between gap-3"><p className="font-medium text-[#102028]">{displayName(item.strategy_id)}</p><span className="text-xs text-slate-500">{currency}</span></div><p className={`mt-4 text-2xl font-semibold tracking-[-0.035em] ${item.metrics.total_return_percent >= 0 ? 'text-[#2d6a57]' : 'text-[#8a5f26]'}`}>{formatPercent(item.metrics.total_return_percent)}</p><p className="mt-1 text-xs text-slate-500">区间变化 · 最大回撤 {formatPercent(-Math.abs(item.metrics.maximum_drawdown_percent))}</p></article>)}</div>
  </section>
}

function ResearchView({ series, currency, effectiveStart, effectiveEnd, contributionCount }: { series: DynamicBacktestSeries[]; currency: string; effectiveStart: string; effectiveEnd: string; contributionCount: number }) {
  return <section className="rounded-[1.45rem] border border-slate-200 bg-white p-5 shadow-sm sm:p-6" aria-label="真实专业回测指标"><div className="flex flex-col gap-3 border-b border-slate-100 pb-5 md:flex-row md:items-end md:justify-between"><div><p className="inline-flex items-center gap-2 text-sm font-medium text-[#2d6a57]"><ShieldCheck className="size-4" />专业研究</p><h2 className="mt-2 text-2xl font-semibold tracking-[-0.035em] text-[#102028]">同一条曲线的风险与资金指标</h2><p className="mt-2 text-sm text-slate-600">{effectiveStart} 至 {effectiveEnd} · 每条策略均投入 {contributionCount} 次</p></div><span className="rounded-full bg-[#e9f2ee] px-3 py-1.5 text-xs font-semibold text-[#2d6a57]">真实 API · {currency}</span></div><div className="mt-5 overflow-x-auto"><table className="w-full min-w-[760px] border-separate border-spacing-0 text-left text-sm"><thead><tr>{['策略', '区间收益', '年化收益', 'XIRR', '最大回撤', '年化波动', 'Sortino', '现金使用率'].map((label) => <th key={label} className="border-b border-slate-200 px-3 py-3 font-medium text-slate-500">{label}</th>)}</tr></thead><tbody>{series.map((item) => <tr key={item.strategy_id} className="text-[#102028]"><td className="border-b border-slate-100 px-3 py-4 font-semibold">{displayName(item.strategy_id)}<span className="mt-1 block text-xs font-normal text-slate-400">v{item.strategy_version}</span></td><Metric value={formatPercent(item.metrics.total_return_percent)} /><Metric value={formatOptionalPercent(item.metrics.annualized_return_percent)} /><Metric value={formatOptionalPercent(item.metrics.xirr_percent)} /><Metric value={formatPercent(-Math.abs(item.metrics.maximum_drawdown_percent))} /><Metric value={formatOptionalPercent(item.metrics.annualized_volatility_percent)} /><Metric value={item.metrics.sortino_ratio?.toFixed(2) ?? '样本不足'} /><Metric value={formatPercent(item.metrics.cash_utilisation_percent)} /></tr>)}</tbody></table></div><div className="mt-5 grid gap-3 md:grid-cols-3"><Fact title="成本口径" text="每次模拟买入计 5 bps 成本；暂不包含税、滑点和最小交易单位。" /><Fact title="因果边界" text="Formula 只读取模拟成交日前已经收盘的数据，不读取当天成交价作决定。" /><Fact title="不是预测" text="这些数字只描述所选历史区间；更换标的、范围或数据修订都会改变结果。" /></div></section>
}
...
function LoadingState() { return <section className="rounded-[1.45rem] border border-slate-200 bg-white p-8" role="status"><div className="h-4 w-40 animate-pulse rounded bg-slate-200" /><div className="mt-5 h-72 animate-pulse rounded-2xl bg-slate-100" /><p className="mt-4 text-sm text-slate-500">正在读取真实日线并统一计算策略…</p></section> }
...
function parseStrategyId(value: string | null): StrategyId | null { return value === 'steady-dca' || value === 'ma200-trend-guard' || value === 'growth-volatility-balance' ? value : null }
function policyId(id: StrategyId): string { return Object.entries(strategyIdByPolicy).find(([, value]) => value === id)?.[0] ?? 'fixed_dca' }
function displayName(policy: string): string { return findConsumerStrategy(strategyIdByPolicy[policy] ?? 'steady-dca').shortName }
