...
> 状态：**工程收口完成，进入发布验证**（2026-09-19 审计）。M0 工程收敛和 M1 的 `Plan → readable Decision → user-reported execution → Audit` 已完成；官方策略目录、真实多市场回测、动态标的建计划、三条策略采用链路与 Formula 创建前数据预检均已落地。下一阶段回到 3–5 位目标用户验证，不把完整调度重构重新变成上线前置条件。
...
2. **官方策略目录与版本化采用**
   - 当前官方目录包含 Fixed DCA、MA200 趋势保护和增长/波动平衡。
...
| 官方策略目录 | Fixed DCA、MA200、增长/波动平衡均来自 `GET /strategy-catalog` | 已完成；DSL 不作为普通入口 |
...
| MA200 趋势保护 | 标的可解析，且具备策略声明所需的有效复权日线窗口 | 创建前预检；评估时读取最近可用 canonical history |
| 增长/波动平衡 | 标的可解析，且具备增长与波动指标所需窗口 | 创建前预检；评估时读取相同数据契约 |
